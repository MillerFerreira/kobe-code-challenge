...
class PopularMoviesViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func setupGradient() {
        ...
        let color0 = UIColor(red:252.0/255, green:219.0/255, blue:84.0/255, alpha:1.0).cgColor
        let color1 = UIColor(red:228.0/255, green:160.0/255, blue: 10.0/255, alpha:1.0).cgColor
    ...
    func setupCollectionView() {
        ...
        popularMoviesCollectionView.register(nibCell, forCellWithReuseIdentifier: "PopularMovieCell")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = popularMoviesCollectionView.dequeueReusableCell(withReuseIdentifier: "PopularMovieCell", for: indexPath) as! PopularMovieCell
            ...
            DispatchQueue.global(qos: .background).async {
                Api.shared.fetchUpcomingMovies(startingFrom: self.loadedPage! + 1, completion: { isSuccessful, movieList in
                    if isSuccessful {
                        ...
                        self.popularMoviesCollectionView.insertItems(at: newMoviesIndexPath)
                    }
                })
            }
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if let movie = movies?[indexPath.item] {
            ...
            performSegue(withIdentifier: "PopularMovie", sender: data)
        }
    }
    ...
    @objc func onNotificationReceived(_ notification: Notification) {
        if let data = notification.object as? NotificationData {
            if data.fromLayout == .Popular {
                self.popularMoviesCollectionView.reloadItems(at: [data.indexPath])
            } else {
